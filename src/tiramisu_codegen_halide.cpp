...
std::set<std::string> allocated_buffers;
    ...
    if (isl_ast_node_get_type(node) == isl_ast_node_block)
    {
        ...
        for (int i = isl_ast_node_list_n_ast_node(list) - 1; i >= 0; i--)
        {
                ...
                (op_type == o_allocate || op_type == o_free || op_type == o_memcpy))
            {
                ...
                {
                    DEBUG(3, tiramisu::str_dump("Adding a computation to vector of allocate stmts (for later construction) [for block]"));
            ...
            else // ((block.defined())
            {
                ...
                if ( !allocate_stmts_vector.empty() )
                {
                    for (auto comp: allocate_stmts_vector)
                    {
                        ...
                        allocated_buffers.insert( buffer_name );
