...
int main(int argc, char **argv)
{
    ...
    // Declare cpu buffers.
    buffer b_A("b_A", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_input);
    buffer b_B("b_B", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_input);
    buffer b_C("b_C", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_output);
    // Declare gpu buffers.
    buffer b_A_gpu("b_A_gpu", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_temporary);
    buffer b_B_gpu("b_B_gpu", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_temporary);
    buffer b_C_gpu("b_C_gpu", {expr(SIZE0), expr(SIZE0)}, p_uint32, a_temporary);
    ...
    // Declare inputs.
    input A("A", {"i", "j"}, {N, N}, p_uint32);
    input B("B", {"i", "j"}, {N, N}, p_uint32);
    ...
    computation C("C", {i,j,k}, p_uint32);
